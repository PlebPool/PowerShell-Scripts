...
}# Set Name.$newName = Read-Host "What will this server be called?"Rename-Computer $newName# Update Windows.Install-Module PSWindowsUpdateGet-WUlist -MicrosoftUpdate$answer = Read-Host "Do you want to install all updates (Y/n)?"if ( ($answer -ne "N") -or ($answer -ne "n") ) {    Install-WindowsUpdate }# Join Domain.# If sql, install server.# Configure SQL, and firewalls.# If SP, install prereqs.# Install SP.# Configure Firewall.# Configure SP. (Create/Join Farm)